...
const Footer: React.FC<FooterProps> = ({ variant = 'default' }) => {
    ...
    return (
        ...
        >
            <div className="container mx-auto px-4">
                <div className="flex flex-col items-center justify-between md:flex-row">
                    ...
                    <div className="text-center text-sm">
                        © {new Date().getFullYear()} Just Create Labs LLC. All
                        rights reserved.
                    </div>

                    <div className="mt-4 flex space-x-4 md:mt-0">
                        <a
                            href="https://linkedin.com/company/just-create-labs-llc/"
                            target="_blank"
                            rel="noopener noreferrer"
            ...
            <button
                onClick={scrollToTop}
                className={`absolute bottom-4 right-4 p-2 ${isCyber ? 'bg-cyber-dark hover:bg-cyber-blue' : 'bg-labs-dark hover:bg-labs-green'} rounded-full transition-colors hover:bg-opacity-10`}
            >
                <ArrowUp
                    className={`h-6 w-6 ${isCyber ? 'text-cyber-blue' : 'text-labs-green'}`}
                />
            </button>
        </footer>
    );
